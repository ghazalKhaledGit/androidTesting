...
class Regester : AppCompatActivity() {
    ...
    fun SaveImageInFirebase() {
        ...
        uploadTask.addOnFailureListener {
            ...
            val vName = URLEncoder.encode(etName.text.toString(), "utf-8")
            val vPassword =URLEncoder.encode(etPassword.text.toString(), "utf-8")
            val vEmail =URLEncoder.encode(etEmail.text.toString(), "utf-8")
            val vImage = URLEncoder.encode(DownloadURL, "utf-8")
            val url = "http://10.0.2.2/CommerceServer/cRegister.php?userName=$vName&userPassword=$vPassword&userEmail=$vEmail&userImage=$vImage"
